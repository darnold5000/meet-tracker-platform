...
# Ensure `cheer_scores/` is importable in environments that don't add the repo root
# to PYTHONPATH (e.g. Streamlit Community Cloud running this as an ASGI app).
sys.path.insert(0, str(PROJECT_DIR))

from cheer_scores.db import fetch_all, fetch_one  # noqa: E402
from app.mvp_api import router as mvp_router  # noqa: E402

# Default HTML meet + `/api/meets` allowlist (`meets.meet_id` in DB). Comma-separated.
ALLOWED_MEET_IDS = [
    m.strip()
    for m in os.getenv("CHEER_SCORES_ALLOWED_MEET_IDS", "").split(",")
    if m.strip()
]
DEFAULT_MEET_KEY = (
    os.getenv("CHEER_SCORES_MEET_KEY", "").strip()
    or (ALLOWED_MEET_IDS[0] if ALLOWED_MEET_IDS else "")
)

_extra_cors = [
    o.strip()
    for o in os.getenv("CHEER_SCORES_CORS_ORIGINS", "").split(",")
    if o.strip()
]
